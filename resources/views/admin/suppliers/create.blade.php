    ...
    <div class="section-body">
        <form method="POST" action="{{ route('superadmin.users.store') }}" id="userCreateForm">
            ...
            <div class="mb-3">
                ...
                <input type="text" id="name" name="name" class="form-control" value="{{ session('form_reset') ? '' : old('name') }}" >
                @error('name') <span class="text-danger small">{{ $message }}</span> @enderror
            </div>

            <div class="mb-3">
                <label for="email" class="form-label">Email Address <span class="text-danger">*</span></label>
                <input type="email" id="email" name="email" class="form-control" value="{{ session('form_reset') ? '' : old('email') }}" >
            ...
            <div class="mb-4">
                ...
                <select name="roles[]" id="roles" class="form-select role-select" multiple>
                    @foreach ($roles as $role)
                        <option value="{{ $role->name }}" {{ session('form_reset') ? '' : (in_array($role->name, old('roles', [])) ? 'selected' : '') }}>
                            {{ ucfirst($role->name) }}
                        </option>
                    @endforeach
                </select>
